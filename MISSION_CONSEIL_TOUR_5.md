# Mission — conseil des modèles, tour 5

*(GPT-5.6 Sol Thinking · Claude Opus 5 Thinking · Gemini 3.1 Pro Thinking)*

---

## 0. Le tour 4 a marché

Vous avez lu le code, et vous avez trouvé. Quatre tours pour y arriver, et la
solution était celle que vous aviez donnée vous-mêmes : coller le texte
inline. C'est acquis, on continue ainsi — **un module par tour, intégral**.

Ce message contient `pfs/solver/pushfold.py`, 681 lignes. C'est le module qui
juge chaque décision de tapis court de l'utilisateur, et donc celui qui rend
la moitié des verdicts qu'il reçoit réellement.

---

## 1. Ce que le tour 4 a produit

Vos trois défauts sont en cours de correction, et je vous dois le détail de
ce qui a été retenu — y compris là où l'un de vous avait tort.

### 1.1 Le seuil PKO : vous avez trouvé le pire défaut du module

`eliminated = s[v] <= 1e-12 * sum(s)`. Instancié sur un MTT réel — 3 337
entrants, 6 685 000 jetons — un vilain à **un jeton** pèse 1,5e-7, soit cinq
ordres de grandeur au-dessus du seuil. Il n'est donc pas vu comme éliminé, la
prime vaut zéro, et l'équité exigée sort fausse.

C'est **exactement** le bug que la docstring dit avoir corrigé — « 90 % au
lieu de 53 % ». Le correctif relatif que nous avions adopté pour satisfaire
votre propre exigence d'invariance d'échelle **a recréé le bug absolu qu'il
devait éliminer**, dans le régime de tapis où l'utilisateur joue vraiment.

C'est le motif le plus instructif de cette consultation : une correction qui
déplace un défaut au lieu de le supprimer, et qui passe tous les invariants
parce qu'elle satisfait celui qu'on venait d'ajouter.

### 1.2 Le partage égal des places restantes

Retenu aussi. Sur `[100,100,0,0]` avec `[50,30,20,10]`, chaque mort reçoit 15.
Mais dans le chemin `bubble_factor`, le héros est **par construction** le
dernier éliminé : il devrait toucher 20. **25 % d'erreur sur `ev_lose`**,
numérateur du facteur de bulle, donc directement sur la pression qui pilote
les ranges push/fold. La correction permet à l'appelant de déclarer l'ordre
quand il le connaît, en gardant le partage égal comme convention d'ignorance.

### 1.3 La dégénérescence DCFR

Votre dérivation est retenue et le test est réécrit : **LCFR à (1,1,1)**,
**CFR+ à (∞, 0, 2)**, **Vanilla CFR comme limite asymptotique inatteignable**.
Notre test affirmait « α=β=γ=1 donne CFR standard », ce qui est faux. Nous
vérifions la dérivation sur notre code plutôt que de la recopier — si notre
implémentation ne suit pas cette paramétrisation, le test serait faux pour
une raison encore différente.

### 1.4 La marche aléatoire absorbante — votre meilleure contribution

À la question « un banc d'invariants peut-il attraper une erreur de
modèle ? », vous avez répondu non, tous les trois, avec la raison
structurelle : échelle, permutation, conservation et monotonie sont
satisfaites par **toute** la famille — Harville, Weitzman, Roberts, et la
marche elle-même. Un banc d'invariants vérifie qu'on est *dans* la famille,
jamais *lequel* on a implémenté.

Et vous avez donné la vérité-terrain que nous cherchions : la **marche
aléatoire absorbante**, dont Malmuth-Harville est une approximation. Elle est
simulable, indépendante de notre implémentation, et l'écart mesuré **est** le
biais de Harville — identifié au tour 1, jamais quantifié. Le banc est en
construction.

C'est l'idée la plus utile des quatre tours, et aucun de nous ne l'avait.

### 1.5 Ce que nous corrigeons aussi

`_validate` tronquait silencieusement les payouts ; les clamps
`min(max(r_pko, 0, 1))` masquaient deux informations fortes — un `r_pko`
négatif signifie « ce call est +EV même à 0 % d'équité », un `r_pko` au-dessus
de 1 signale une erreur de saisie ; l'erreur-type du Monte-Carlo était une
borne jamais rendue à l'appelant ; les `lru_cache` sur closures ne servaient
à rien entre appels ; la rotation du bouton du FGS dérivait après élimination.

### 1.6 Et sur le reste du logiciel

Vos objections du tour 3 ont produit des mesures. Le « 95 % » de lecture des
cartes valait en réalité **76,7 %** de rappel, dont **65,1 %** avec le bon
rôle — mais **100 % de précision** sur 199 cartes affirmées, et zéro carte
inventée. Vous aviez raison sur la métrique, et l'annotation manuelle des
57 captures l'a établi.

Trois faux positifs confiants ont été trouvés et fermés, tous invisibles sans
données réelles. Le plus instructif : une carte en retournement était
**correctement refusée** par le contrôle de couleur, mais ce refus était
**muet** — la recherche de cadrage prenait le relais et finissait par trouver
un cadrage acceptable. Un garde-fou qu'on contourne en changeant de chemin
n'en est pas un.

---

## 2. Ce que nous vous demandons — `pushfold.py`

Le code suit au §5. Même format : **CONFIRMÉ / RÉFUTÉ / NON VÉRIFIÉ**.

1. **L'équilibre de Nash push/fold est-il correct ?** La méthode de
   résolution, la convergence, et la façon dont les tables sont produites ou
   interpolées. Confrontez à la table Nash HU de HoldemResources — **pas** à
   SAGE, dont vous nous avez signalé qu'elle n'est pas Nash.
2. **L'articulation avec l'ICM.** Le module produit-il un équilibre chipEV
   qu'on corrige ensuite, ou résout-il directement sous ICM ? Les deux sont
   défendables ; lequel est fait, et est-ce cohérent avec ce que le
   conseiller en tire ?
3. **La gestion des antes.** C'est là que les erreurs se cachent en tournoi
   moderne : ante du bouton, ante de chaque joueur, `ante_trimming_status`.
   Vérifiez que le pot mort est calculé juste.
4. **Le domaine de validité.** HoldemResources note que le push/fold pur est
   déconseillé au-delà de ~20 bb. Que fait notre module à 25 bb ? Un calcul
   exact assorti d'un conseil inapplicable est la même famille de défaut que
   celle que vous avez trouvée au tour 3.
5. **Les 169 groupes de mains.** L'isomorphisme assorti/dépareillé, et la
   façon dont les fréquences mixtes sont rendues — ou écrasées.

---

## 3. Là où nous avons encore besoin d'aide

### 3.1 La propagation par intervalles — votre synthèse est retenue, une question demeure

Nous adoptons l'échantillonnage des **sommets** plutôt que l'arithmétique
d'intervalles, sur quatre entrées au plus — pot lu, mise lue, tapis effectif,
choix de modèle — soit 16 solves. La justification est celle d'Opus 5 :
l'ICM étant monotone en chaque tapis, les extrema sont aux sommets. Et
`fragile` sera gradué en trois niveaux plutôt que binaire.

La question ouverte : **le coût**. Les `lru_cache` actuels ne servent à rien
entre appels et `bubble_factor` fait déjà trois solves. Seize solves par
verdict, sur une table finale à neuf joueurs en exact — est-ce tenable, et
sinon, quelle approximation accepteriez-vous en premier : réduire le nombre
d'entrées, passer en Monte-Carlo, ou ne propager que sur les verdicts
marginaux détectés par une première passe scalaire ?

### 3.2 Le biais de Harville, une fois mesuré, on en fait quoi ?

Le banc de marche aléatoire va nous donner un écart chiffré. Ensuite :
corriger vers la marche, proposer les deux modèles, ou ne signaler que les
verdicts qui **basculent** entre les deux ? Cette dernière option est la plus
cohérente avec notre positionnement — admettre l'ignorance — mais elle
...
prouver qu'on joue mieux à partir du **résultat** d'une main, et le corpus
donne toutes les cartes de tous les joueurs, ce qui rend le sophisme du
résultat encore plus tentant.

Nous avons donc retenu trois catégories : **erreur prouvable sans le recul**
(la décision est mauvaise contre toute range plausible), **écart avec le
recul seul** (défendable au moment de jouer — ce n'est pas une erreur), et
**désaccord sans preuve**.

Est-ce la bonne partition ? Quel critère supplémentaire rendrait la première
catégorie plus rigoureuse ? Et sur 83 mains de tables finales à 50 000 $
d'entrée, quel nombre d'erreurs prouvables vous paraîtrait **suspect par
excès** ?
...
Deux sections obligatoires : **« ce que je n'ai pas pu vérifier »** et
**« mon désaccord principal »**. Répondez séparément.

Sur quatre tours, la bonne réponse a été cinq fois celle d'un seul d'entre
vous : le biais de Harville, le piège SAGE, l'erreur d'équité de Gemini, la
fausse coïncidence des 31,7 %, et le seuil PKO instancié sur un vrai MTT.
Aucun avis unique ne les aurait tous.

---

## 5. Le code — `pfs/solver/pushfold.py`, intégral

```python
# ═══ pfs/solver/pushfold.py ═══ (796 lignes, intégral)
...
## 6. La question de fond, reposée

Au tour 4 vous avez répondu qu'un banc d'invariants ne peut jamais attraper
une erreur de modèle, et vous nous avez donné la marche aléatoire absorbante
comme vérité-terrain de l'ICM.

**Quelle est la vérité-terrain d'un équilibre de Nash push/fold ?**

Concorder avec HoldemResources ne prouverait que la reproduction fidèle d'un
autre solveur. Le critère interne — l'exploitabilité, c'est-à-dire le gain de
la meilleure réponse contre notre stratégie — est calculable et indépendant
de toute référence externe. Est-ce suffisant ? Un équilibre exact a une
exploitabilité nulle par définition, mais une implémentation peut avoir une
exploitabilité nulle *dans son propre modèle du jeu* tout en modélisant le
mauvais jeu — antes mal comptés, blindes mal placées, ordre d'action faux.

Autrement dit : quelle est ici l'équivalent de la marche aléatoire — la chose
extérieure au calcul qui dirait qu'on résout le bon jeu, et pas seulement
qu'on le résout bien ?
